...
public class FullTest {
	...
	private class SA extends Thread {
		...
		public void run() {
			try {
				...
				Advertiser adv = ServiceLocationManager
						.getAdvertiser(java.util.Locale.getDefault());

				for (int i = 0; i < _iter; i++) {
					Vector attr = new Vector();
					Vector values = new Vector();
					values.add(new Integer((int) Math.round(Math.random()
							* Integer.MAX_VALUE)));
					attr.add(new ServiceLocationAttribute("Attribute", values));

					ServiceURL url = new ServiceURL("service:slpTest" + (i % 5)
							+ "://" + (i % 255) + "." + (i % 255) + "."
							+ (i % 255) + "." + (i % 255) + ":" + i, _lifetime);
	...
	private class UA extends Thread {
		...
		public void run() {
			try {
				Locator locator = com.solers.slp.ServiceLocationManager
						.getLocator(java.util.Locale.getDefault());
				Vector scopes = new Vector();
				scopes.add("default");

				for (int i = 0; i < _iter; i++) {
					ServiceLocationEnumeration e = locator.findServiceTypes("",
							scopes);
					while (e.hasMoreElements()) {
						ServiceType type = (ServiceType) e.next();
						System.out.println(type.toString());
						ServiceLocationEnumeration enum2 = locator
								.findServices(type, scopes, "");
						while (enum2.hasMoreElements()) {
							ServiceURL url = (ServiceURL) enum2.next();
							System.out.println("\t" + url.toString());
							ServiceLocationEnumeration enum3 = locator
									.findAttributes(url, scopes, new Vector());
