...
typedef enum {EQUAL, APPROX, GREATER, LESS, PRESENT, SUBSTRING} Operation;
...
SLPError filter(char *start, char **end, SLPAttributes slp_attr, SLPBoolean *return_value, int recursion_depth) {
	...
	if (IS_VALID_TAG_CHAR(*cur)) {
		...
		if (operator == cur) {/* Check that we can poke back one char. */
			return SLP_PARSE_ERROR;
		}
		...
		if (slp_err == SLP_TAG_ERROR) { /* Tag  doesn't exist. */
			*return_value = SLP_FALSE;
			err = SLP_OK;
		} 
		else if (slp_err == SLP_OK) { /* Tag exists. */
			/**** Do operation. *****/
			if (op == PRESENT) { 
		...
		else { /* Some other tag-related error. */
